/*
    ...
    Started: 2016-4-20
    Notes:
        User-independent utilities for the host application.    
*/
...
public:
    ...
    {
        PrintFunction();
        return uint32_t(std::chrono::duration_cast<std::chrono::seconds>(std::chrono::system_clock::now().time_since_epoch()).count() - Steam_StartupTimestamp);
    }
    uint32_t GetSecondsSinceComputerActive()
    {
        PrintFunction();
        return uint32_t(std::chrono::duration_cast<std::chrono::seconds>(std::chrono::system_clock::now().time_since_epoch()).count() - Steam_StartupTimestamp + 600);
    }
