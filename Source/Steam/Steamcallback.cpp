/*
    ...
    Started: 2016-4-20
    Notes:
        Implementation of Steams async fetch requests.
*/
...
{
    DebugPrint(va_small("%s <%s>", __FUNCTION__, GetCallbackName(callback)));
...
{
    DebugPrint(va_small("%s <%s> (%lli)", __FUNCTION__, GetCallbackName(result->GetICallback()), call));
...
{
    DebugPrint(va_small("%s <%lli>", __FUNCTION__, call));
...
uint64_t SteamCallback::RegisterCall()
{
    Threadsafe.lock();
    {
        ...
        DebugPrint(va_small("%s <%i>", __FUNCTION__, _CallID));
    }
    Threadsafe.unlock();
    return _CallID;
}
void SteamCallback::ReturnCall(void* data, int32_t size, int32_t type, uint64_t call)
{
    ...
    Threadsafe.lock();
    {
        ...
        DebugPrint(va_small("%s <%lli, %s>", __FUNCTION__, call, GetCallbackName(type)));
